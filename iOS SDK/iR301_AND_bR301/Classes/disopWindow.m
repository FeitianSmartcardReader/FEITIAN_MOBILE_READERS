...
/*Upadte UI in main thread*/
-(void)changeCardState
{
    [cardState setOn:cardIsAttached];
    if (cardIsAttached == FALSE) {
        //After card removed, power off to card and clear the text view
        disTextView.text = @"";
...
- (void)viewDidLoad {
    ...
    //Set delegate, using to monitor card and reader status
...
//Display reader device information and SDK version
...
//Show the info of card and reader
-(IBAction) showInfo
{
...
-(IBAction) powerOnFun:(id)sender
{
    ...
    SCardReconnect(gCardHandle, SCARD_SHARE_SHARED, SCARD_PROTOCOL_T0, 0, &dwActiveProtocol);
	if (iRet != 0) {
