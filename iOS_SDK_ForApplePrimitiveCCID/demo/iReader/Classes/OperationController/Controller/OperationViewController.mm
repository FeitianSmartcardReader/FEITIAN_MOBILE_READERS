...
- (IBAction)disconnectCard:(id)sender {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        NSLog(@"SCardDisconnect-------ret--%ll@",ret);
        [[Tools shareTools] hideMsgView];
...
//
    ...
    if(([FTDeviceType getDeviceType] & LINE_TYPEC) != LINE_TYPEC)
    {
        dispatch_sync(dispatch_get_main_queue(), ^{
            self.powerOnBtn.hidden = NO;
            self.powerOffBtn.hidden = NO;
        });
        ...
        dispatch_sync(dispatch_get_main_queue(), ^{
            self.powerOnBtn.hidden = YES;
            self.powerOffBtn.hidden = YES;
        });
    }
...
- (long)sendCommand:(NSString *)apdu
{
    ...
    unsigned char capdu[8192 + 128];
    memset(capdu, 0, 8192 + 128);
    unsigned char resp[8192 + 128];
    memset(resp, 0, 8192 + 128);
...
- (void)cardInterfaceDidDetach:(BOOL)attached slotname:(NSString *)slotname
{
    if (attached) {
        ...
        if(([FTDeviceType getDeviceType] & LINE_TYPEC) != LINE_TYPEC)
        {
            ...
            NSLog(@"getDeviceType===%u",[FTDeviceType getDeviceType] );
            dispatch_async(dispatch_get_global_queue(0, 0), ^{
                [self connectCard:nil];
            });
        }
